...
### 9.1 Deploying on Railway
1. **Create Repo:** Push the project repository to your GitHub account.
2. **Setup Railway:** Log into the Railway Dashboard and click **New Project** → **Deploy from GitHub repo**.
3. **Connect Repository:** Select your repository branch.
4. **Define Variables:** Under the service **Variables** tab, set:
  - `SUPABASE_URL`
  - `SUPABASE_KEY`
  - `SECRET_KEY`
5. **Port Configuration:** Railway automatically binds the port. The `Procfile` command `web: gunicorn app:app` starts the production server.
6. **Automatic Build:** Railway automatically builds, deploys, and assigns a public domain to the project.
