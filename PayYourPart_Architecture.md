...
### 9.1 Backend Web App Deployment (Railway)

The core web app containing authentication, groups ledger, splits engine, and settlements is deployed to Railway.

1. **GitHub Repository**: Push the project code to your GitHub account.
2. **Create Railway Service**: Go to the [Railway Dashboard](https://railway.app), click **New Project** → **Deploy from GitHub repo**, and select your repository.
3. **Configure Settings**:
   - Railway will automatically detect the `Procfile` and set the start command to `gunicorn app:app`.
4. **Configure Environment Variables**:
   - Go to the service **Variables** tab and set the following variables:
     - `SUPABASE_URL` = (Your Supabase project URL, e.g., `https://your-project.supabase.co`)
     - `SUPABASE_KEY` = (Your Supabase Anon API Key)
     - `SECRET_KEY` = (A secure random string for Flask cookie session encryption)
     - `FRONTEND_URL` = (The URL of your deployed Next.js marketing page, e.g., `https://payyourpart.vercel.app`)
5. **Domain Assignment**: Go to service **Settings** → **Domains** and generate/assign a public domain (e.g., `https://payyourpart-production.up.railway.app`). Note this URL for the frontend setup.
...
3. **Configure Environment Variables**:
   - Add the following environment variable to link the landing page action buttons to the backend web app:
     - `NEXT_PUBLIC_BACKEND_URL` = (The public URL generated by Railway in Step 5 of the backend setup, e.g., `https://payyourpart-production.up.railway.app`)
4. **Deploy**: Click **Deploy**. Vercel will build the Next.js site and assign it a production URL (e.g., `https://payyourpart.vercel.app`).
5. **Update Backend**: If you deployed the frontend after the backend, copy your Vercel domain and update the `FRONTEND_URL` variable in your Railway dashboard settings.
...
1. **Supabase Redirects**: In your Supabase Dashboard, navigate to **Auth** → **URL Configuration** and update:
   - **Site URL**: Point this to your live Railway domain (e.g., `https://payyourpart-production.up.railway.app`).
   - **Redirect URLs**: Add `https://payyourpart-production.up.railway.app/**`.
